...
"""Capture the video BOX-prompt golden from the OFFICIAL SAM 3.1 multiplex.

Run ONCE in the isolated reference env (NOT this repo's pixi env):

    cd C:/Users/javerlia/PycharmProjects/sam3_reference
    ./.venv/Scripts/python.exe \
        ../sam2/tests/parity/reference_sam3/capture_sam3p1_video_box_golden.py \
...
Scenario (video BOX prompt, NO text concept):
  Stream the first N=8 bedroom frames (0..7). On frame 0 add ONE BOX prompt
  (no text_str). The box, in NATIVE pixels xyxy = [300, 150, 470, 420] on the
  960x540 video, is converted to the normalized [xmin, ymin, width, height]
  the upstream box API expects:
      xmin = 300/960,  ymin = 150/540,
      width = (470-300)/960,  height = (420-150)/540
  The box drives detection via the GEOMETRIC slot (frame-0 text stays the
  "<text placeholder>"/geometric default). The detector biases toward the
  boxed person; detections seed tracklets via normal association (fresh ids).
  Then propagate forward and record, per frame, the upstream out_obj_ids and
  each object's binary mask at native 540x960.
...
API path: MODEL API (Sam3MultiplexTrackingWithInteractivity, predictor.model).
  ...
  then drive the underlying model directly (predictor start_session/handle_request
  is broken in this reference rev).

  Upstream box add_prompt (sam3_multiplex_tracking.py:1654-1737, box branch
  1706-1728 + _get_visual_prompt 186-204). text_str is NOT required -- the
  add_prompt assert only needs one of text/points/boxes:

      model = predictor.model
      state = model.init_state(resource_path=..., async_loading_frames=False)
      model.add_prompt(
          state, frame_idx=0,
          boxes_xywh=<(1, 4) normalized [xmin, ymin, width, height] in 0..1>,
          box_labels=<(1,) long, value 1>,
      )                                   # NO text_str
  ...
  Propagate FROM frame 0 (unlike the text modelfind golden, which starts at 1):
  a box-only add_prompt records NO "add" action, so propagate runs a FULL VG
  pass, and the box's geometric prompt exists ONLY on frame 0. Propagation must
  therefore process frame 0 to detect the boxed object and seed a tracklet that
  the SAM2 tracker carries forward; starting at frame 1 skips the box entirely
  and yields nothing. We keep add_prompt's authoritative frame-0 detection and
  take frames 1..N-1 from propagation. NOTE: the built multiplex model uses
  hotstart_delay=15; with only N=8 frames the whole clip is in the hotstart
  warm-up window, so the buffer flushes at the last frame and the box object is
  surfaced only after it clears hotstart confirmation (see per-frame numbers in
  the run log). This warm-up regime is the genuine upstream behaviour for a
  short clip -- it is the golden truth, not an artifact.
...
def main():
    ...
    # Native pixel xyxy -> normalized [xmin, ymin, width, height] in 0..1.
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="sam3p1_video_box_ref_"))
    ...
    try:
        ...
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                # Propagate FROM frame 0 (not 1): the box's geometric prompt lives
                # on frame 0, so full-VG propagation must process frame 0 to seed the
                # tracklet, which the SAM2 tracker then carries forward. Starting at 1
                # would skip the box detection entirely and yield nothing downstream.
                # We keep add_prompt's authoritative frame-0 detection (out0) and take
                # frames 1..N-1 from propagation (frame 0's propagation re-yield is
                # empty here because the object is not yet hotstart-confirmed).
                ...
                for f_idx, out in model.propagate_in_video(
                    ...
                    if f_idx >= args.n:
                        break
                    if f_idx == 0:  # keep add_prompt's box detection for frame 0
                        continue
                    _collect(f_idx, out)
    finally:
