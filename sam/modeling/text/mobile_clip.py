# SPDX-License-Identifier: LicenseRef-AppleML
# Copyright (C) 2024 Apple Inc. All Rights Reserved.
# Modified for EfficientSAM3 by SimonZeng7108/efficientsam3 @ d063e00.
# Vendored into sam/modeling/text/ (Phase A5) with the following de-timm changes:
#   * timm ``DropPath`` -> ``sam.modeling.encoders.pe_vitdet.DropPath``
#   * timm ``trunc_normal_`` -> ``torch.nn.init.trunc_normal_``
# MobileCLIP is licensed under the Apple Machine Learning Research License;
# see https://github.com/apple/ml-mobileclip for the original source.
...
from sam.modeling.encoders.pe_vitdet import DropPath
...
class RepMixerBlock(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        if x.dim() == 3:
            # B, seq, dim -> B, dim, 1, seq  (for Conv2d)
            x = x.permute(0, 2, 1)
        ...
        # Convert back: B, dim, 1, seq -> B, seq, dim
...
class MobileCLIPTextTransformer(nn.Module):
    ...
    def encode_text(
        ...
        **kwargs,
        ...
        if input_is_embeddings:
            # Fallback to last token if not returning all tokens
