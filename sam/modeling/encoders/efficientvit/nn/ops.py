...
# Vendored from SimonZeng7108/efficientsam3 @ d063e00 (sam3/backbones/efficientvit/nn/ops.py); intra-package imports rewritten.
# Upstream source: MIT-HAN-Lab/efficientvit (Apache-2.0)
# Change from upstream: `from sam3.device import get_autocast_device_type` replaced with
# an inline definition of the same function to avoid the sam3 package dependency.
...
def _get_autocast_device_type(device) -> str:
    """Return device type string suitable for ``torch.autocast`` (cuda/mps/cpu)."""
...
class UpSampleLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if (self.size is not None and tuple(x.shape[-2:]) == self.size) or self.factor == 1:
            return x
        device_type = _get_autocast_device_type(x.device)
...
class LiteMLA(nn.Module):
    ...
    def relu_linear_att(self, qkv: torch.Tensor) -> torch.Tensor:
        ...
        device_type = _get_autocast_device_type(qkv.device)
        with torch.autocast(device_type=device_type, enabled=False):
