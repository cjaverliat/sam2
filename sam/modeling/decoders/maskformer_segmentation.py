...
# (sam3/model/maskformer_segmentation.py): the base (PER-OBJECT) mask head -- a
# ``PixelDecoder`` (Simple-FPN upsampler over the vision pyramid, with the principal level
# replaced by the fused encoder memory) + a per-query ``MaskPredictor`` (MLP x pixel-embed
# einsum). This is the NON-multiplex head used by base ``sam3.pt``; the SAM 3.1 multiplex
# mask decoder is intentionally NOT vendored here.
#
# Stripped: the NestedTensor (multiplex) unwrap, activation checkpointing, and torch.compile.
# Attribute names are preserved so ``detector.segmentation_head.*`` loads with strict=True.
"""SAM 3 per-object segmentation head (PixelDecoder + UniversalSegmentationHead)."""
...
from sam.modeling.decoders.detr_decoder import MLP
...
class PixelDecoder(nn.Module):
    def __init__(self, hidden_dim, num_upsampling_stages, interpolation_mode="nearest",
                 shared_conv=False, **kwargs):
        super().__init__()
    ...
    def forward(self, backbone_feats: List[torch.Tensor]):
        # backbone_feats are already projected (C == hidden_dim); coarsest level last.
...
class SegmentationHead(nn.Module):
    def __init__(self, hidden_dim, upsampling_stages, use_encoder_inputs=False,
                 aux_masks=False, no_dec=False, pixel_decoder=None, shared_conv=False):
        super().__init__()
        ...
        if self.use_encoder_inputs:
            if backbone_feats[0].shape[0] > 1:
                backbone_visual_feats = [
                    feat[image_ids.to(feat.device), ...].to(model_device)
...
class UniversalSegmentationHead(SegmentationHead):
    """Per-object instance segmentation (+ a semantic head + an optional presence head)."""

    def __init__(self, hidden_dim, upsampling_stages, pixel_decoder, aux_masks=False,
                 no_dec=False, presence_head=False, dot_product_scorer=None,
        ...
        if dot_product_scorer is not None:
            assert presence_head, "dot product scorer without a presence head is a mistake"
        self.presence_head = None
        if presence_head:
            self.presence_head = (
                dot_product_scorer if dot_product_scorer is not None
    ...
    def forward(self, backbone_feats, obj_queries, image_ids, encoder_hidden_states=None,
                prompt=None, prompt_mask=None, **kwargs) -> Dict[str, Optional[torch.Tensor]]:
        ...
        if self.cross_attend_prompt is not None:
            ...
            tgt2 = self.cross_attend_prompt(
                query=tgt2, key=prompt, value=prompt, key_padding_mask=prompt_mask
            )[0]
        ...
        if self.presence_head is not None:
            ...
            presence_logit = (
                self.presence_head(
                    pooled_enc.view(1, bs, 1, self.d_model), prompt=prompt, prompt_mask=prompt_mask
                )
                .squeeze(0)
                .squeeze(1)
            )
        ...
        pixel_embed = self._embed_pixels(
            backbone_feats=backbone_feats, image_ids=image_ids,
